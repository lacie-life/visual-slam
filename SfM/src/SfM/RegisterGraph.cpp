...
namespace SimpleSfM
{
    ...
    double RegisterGraph::GetMeanNumTrial()
    {
        ...
        // TODO : Improve here, if the picture is not fully registered, then the denominator is incorrect
    ...
    std::vector<image_t> RegisterGraph::GetNextImageIds()
    {
        ...
        {
            // Skip registered pictures
            if (IsRegistered(image_id))
                continue;

            // If the neighbor is not registered then also skip
