...
namespace SimpleSfM
{
    void SceneGraph::Load(const cv::Ptr<Database> database, const size_t min_num_matches)
    {
        ...
        const std::vector<std::pair<image_pair_t, std::vector<cv::DMatch>>> image_pairs = database->ReadAllMatches();
        ...
        /// add pictures
        // TODO : Improve the following logic, because it will conflict with register graph
    ...
    void SceneGraph::Finalize()
    {
        for (auto it = images_.begin(); it != images_.end();)
        {
            ...
            // Indicates that the feature points of this image do not match the feature points of other images
            // Indicates that the image is isolated
            // delete the image
