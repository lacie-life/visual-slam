...
namespace SimpleSfM
{
                            ...
                            const std::vector<cv::Vec2d> &points2D2)
    {
        ...
        // No matter which method is used for initialization, the number of internal points must meet the requirements
        if (H_F_ratio < 0.7 && num_inliers_F >= params_.rel_pose_min_num_inlier)
        {
            // Initialize with the underlying matrix
                                                ...
                                                const std::vector<bool> &inlier_mask_H)
    {
        ...
        // Find the most suitable solution among these solutions
                                                  ...
                                                  const std::vector<bool> &inlier_mask_F)
    {
        ...
        // due to the use of E = K_.t() * F * K_
        // then recoverPose will fail
        // So use opencv's findEssentialMat directly
        // Then recoverPose
        ...
        for (size_t i = 0; i < points2D1.size(); ++i)
        {
            ...
            // The points obtained by triangulation must satisfy
            // positive depth
            // The reprojection error is less than the threshold
            // The angle of the triangulation is greater than the threshold
        ...
        // Gets the median of the angles of 3D points that have been successfully triangulated (i.e. the reprojection error is less than a certain threshold)
        if (tri_angles.size() % 2 == 1)
        {
            median_tri_angle = tri_angles[tri_angles.size() / 2];
        }
                                       ...
                                       const cv::Vec2d &point2D2)
    {
        ...
        // From homogeneous coordinates -> non-homogeneous coordinates
